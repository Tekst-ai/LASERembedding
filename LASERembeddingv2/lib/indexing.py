...
def IndexCreate(
    dname, idx_type, verbose=False, normalize=True, save_index=False, dim=1024
):

    assert idx_type == "FlatL2", "only FlatL2 index is currently supported"
    x = np.fromfile(dname, dtype=np.float32, count=-1)
    nbex = x.shape[0] // dim
    print(" - embedding: {:s} {:d} examples of dim {:d}".format(dname, nbex, dim))
    x.resize(nbex, dim)
...
def IndexSearchMultiple(
    data, idx, langs, verbose=False, texts=None, print_errors=False
):
    nl = len(data)
    nbex = data[0].shape[0]
    ...
    for i1 in range(nl):
        for i2 in range(nl):
            if i1 != i2:
                    ...
                    for p in range(I.shape[0]):
                        if texts[i2][p] != texts[i2][I[p, 0]]:
                            ...
                            if print_errors:
                                print(
                                    "Error {:s}\n      {:s}".format(
                                        texts[i2][p].strip(), texts[i2][I[p, 0]].strip()
                                    )
                                )
                    err[i1, i2] = e1 / nbex
                ...
                if verbose:
                    print(
                        " - similarity error {:s}/{:s}: {:5.2f}%".format(
                            langs[i1], langs[i2], 100.0 * err[i1, i2]
                        )
                    )
    return err
